...
figlet CLEANING YOUR LENS -c | lolcat
printf "\n"
...
command -v php > /dev/null 2>&1 || { echo >&2 "shadowlens require php but it's not installed. Install it. Aborting."; exit 1; }
...
printf "\n"
echo -e "\nWaiting for the targets to open the link ..." | lolcat
echo -e "\nPress ctrl+c to exit ... " | lolcat
while [ true ]; do
...
command -v ssh > /dev/null 2>&1 || { echo >&2 "Shadowlens require ssh but it's not installed. Install it. Aborting."; exit 1; }
...
read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] \e[0m' option_tem
...
command -v php > /dev/null 2>&1 || { echo >&2 "Shadowlens require ssh but it's not installed. Install it. Aborting."; exit 1; }
start
